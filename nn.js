class NeuralNetwork {
  ...
  static async loadModel(jsonFile, weightsFile) {
    const jsonUpload = document.createElement('input');
    jsonUpload.type = 'file';
    jsonUpload.accept = '.json';
    jsonUpload.style.display = 'none';
    document.body.appendChild(jsonUpload);

    const weightsUpload = document.createElement('input');
    weightsUpload.type = 'file';
    weightsUpload.accept = '.bin';
    weightsUpload.style.display = 'none';
    document.body.appendChild(weightsUpload);

    return new Promise((resolve, reject) => {
      jsonUpload.onchange = async () => {
        const jsonFile = jsonUpload.files[0];
        weightsUpload.onchange = async () => {
          const weightsFile = weightsUpload.files[0];
          try {
            const model = await tf.loadLayersModel(tf.io.browserFiles([jsonFile, weightsFile]));
            resolve(new NeuralNetwork(model, model.layers[0].inputShape[1], model.layers[0].units, model.layers[1].units));
          } catch (error) {
            reject(error);
          }
        };
