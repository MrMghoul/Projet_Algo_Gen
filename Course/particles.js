function pldistance(p1, p2, x, y) {
  ...
  class Particle {
    ...
    look(walls) {
      ...
      const xs = tf.tensor2d([inputs]); // Convertir les entrées en un seul tenseur
      const output = this.brain.predict(xs).dataSync(); // Prédire la sortie et récupérer les valeurs
      xs.dispose(); // Libérer la mémoire du tenseur d'entrée
